...
class ImageSender():
    ...
    send images or send jpg self.compressed images.
...
class ImageHub():
    ...
    def __init__(self, open_port='tcp://*:5555', REQ_REP = True,conflate=True):
    ...
    def recv_jpg(self, copy=False):
        ...
        Returns:
          ...
          jpg_buffer: bytestring jpg self.compressed image
        """

        msg, jpg_buffer = self.zmq_socket.recv_jpg(copy=False)
        return msg, jpg_buffer

    def recv_any(self, copy=False):
        """Receives text msg, jpg buffer.
        ...
        msg, any_buffer = self.zmq_socket.recv_any(copy=False)
        return msg, any_buffer
...
class SerializingSocket(zmq.Socket):
    ...
    Also used for sending / receiving jpg self.compressed OpenCV images.
    """
    compress=False
    ...
    def recv_any(self, flags=0, copy=False, track=False):
        ...
        if 'dtype' in data.md:
            if self.compress:
                ...
                d = data.img
            A = np.frombuffer(d, dtype=data.md['dtype'])
            A = A.reshape(data.md['shape'])
        else:
            A = data.img
        return (data.md['msg'], A)
